...
__global__ void ClusteredRandomWalk_get1page_kernel(int Nthreads, int *d_step_counts){
	...
	if (tid<Nthreads){
		int step_counts;
		int pageID = getPageClusteredRandomWalk(&step_counts);
		freePageClusteredRandomWalk(pageID);
		pageID = getPageClusteredRandomWalk(&step_counts);
 ...
 */
Metrics_t runClusteredRandomWalk(int Nthreads){
	...
	ClusteredRandomWalk_get1page_kernel <<< ceil((float)Nthreads/32), 32 >>> (Nthreads, d_step_counts);
...
int main(int argc, char const *argv[])
{
	...
	for (int i=0; i<TOTAL_N_PAGES/Nthreads; i++){
		...
		Metrics_t metrics = runClusteredRandomWalk(Nthreads);
