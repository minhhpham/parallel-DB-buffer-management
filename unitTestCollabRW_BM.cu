...
__global__ void CollabRW_BM_get1page_kernel(int Nthreads, int *d_step_counts){
 ...
 */
Metrics_t runCollabRW_BM(int Nthreads, int NFree){
	...
	CollabRW_BM_get1page_kernel <<< ceil((float)Nthreads/32), 32 >>> (Nthreads, d_step_counts);
	gpuErrchk( cudaPeekAtLastError() );
	gpuErrchk( cudaDeviceSynchronize() );
...
int main(int argc, char const *argv[])
{
	...
	if(argc>1 && ((strncmp(argv[1], "-h", 2) == 0) || (strncmp(argv[1], "-help", 4) == 0))){
		fprintf(stderr, "USAGE: ./unitTestCollabRW_BM [options]\n");
