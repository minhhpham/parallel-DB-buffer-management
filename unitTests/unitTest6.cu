/* 
	perform one-page allocation on only thread 0-15 of each warp, collect TAS, WAS, time
...
__global__ void get1page_kernel(int Nthreads, int *d_step_counts){
    ...
    int step_counts;
	if (tid<Nthreads && (threadIdx.x%32<16)){
		int *step_counts_ptr = d_step_counts ? &step_counts : nullptr;
		auto pageID = getPage(step_counts_ptr);
	}
    // copy step_counts to the other half-warp so we can get correct measurements later
    __syncwarp();
    step_counts = __shfl_up_sync(__activemask(), step_counts, 16);
